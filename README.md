...
~~如果本次 Markdown 由你生成或主要改写，请在 commit message 中加入 [md:ai]。~~
...
目前已经可以确定，GitHub 的 `$$ ... $$` 并不能可靠地把内部内容完全隔离于 Markdown 的块级解析；而 ` ```math ... ``` ` 是真正的 fenced block。已经实际测试过，同一段包含空行的公式使用 `$$` 无法渲染，改成 ` ```math ` 后即可正常显示，因此块公式统一优先转换为后者。
...
~~不太重合，因此处理时会区分人工在 Typora 中编写和 AI 直接生成 Markdown 的情况。~~

~~默认按照人工编写处理，AI 提交时特别标注就可以触发 AI 情况。~~

后来实际实现后发现这些特定规则本身都很少，统一执行也不会增加新的扫描轮次，因此最终不再区分来源。
